...
Tree / Prop / Terrain 当前仍按 Alpha 安全策略 fail-closed；Citizen / Vehicle / Path 的完整自然模拟 authority 与生产级公网认证传输也不是本 DLC 矩阵的完成条件。它们属于完整 1.0 的基础游戏/网络层工作，不应和“某个 DLC 未分类”混为一谈。
