...
namespace CsmForge.Core
{
    ...
    public sealed class BuildingIntentV2
    {
        ...
        public static BuildingIntentV2 Create(string prefabKey, float x, float y, float z, float angle, byte length)
        { return Create(prefabKey, x, y, z, angle, length, 0); }
        ...
        public static BuildingIntentV2 Create(string prefabKey, float x, float y, float z, float angle, byte length, int constructionCost)
        {
            ...
            return new BuildingIntentV2 { Kind = BuildingIntentKindV2.Create, PrefabKey = prefabKey,
                X = x, Y = y, Z = z, Angle = angle, Length = length, ConstructionCost = constructionCost };
        }
        ...
        internal static void ValidatePrefab(string value)
        {
            if (string.IsNullOrEmpty(value) || Encoding.UTF8.GetByteCount(value) > 192)
                throw new ArgumentException("Invalid building prefab identity.", "value");
        }
    ...
    public sealed class BuildingResultV2
    {
        ...
        public static BuildingResultV2 Deleted(EntityIdentityV2 entity)
        {
            if (!entity.IsValid) throw new ArgumentException("Invalid building entity.", "entity");
            return new BuildingResultV2 { Kind = BuildingResultKindV2.Deleted, Entity = entity };
        }
    }

    /// <summary>
    /// Pure canonical projection of the currently supported building-placement closure.
    /// BuildIndex and ConstructionCost are intentionally not part of the long-lived root: both
    /// are operation/projection metadata and are not required to reconstruct a loaded building.
    /// </summary>
    public sealed class BuildingStateIndexV2
    {
        ...
        {
            if (state == null) throw new ArgumentNullException("state");
            BuildingStateV2 current;
            if (states.TryGetValue(state.Entity.EntityId, out current))
            {
                if (!current.Entity.Equals(state.Entity)) throw new InvalidOperationException("Entity generation conflict.");
                throw new InvalidOperationException("Building entity already exists.");
            }
        ...
        public void Apply(BuildingResultV2 result)
        {
            ...
            if (result.Kind == BuildingResultKindV2.Created) { Seed(result.State); return; }
            BuildingStateV2 current;
            if (!states.TryGetValue(result.Entity.EntityId, out current) || !current.Entity.Equals(result.Entity))
                throw new InvalidOperationException("Cannot delete an unknown or stale building entity.");
            states.Remove(result.Entity.EntityId);
        }
        ...
        public bool TryGet(EntityIdentityV2 entity, out BuildingStateV2 state)
        {
            ...
            BuildingStateV2 value;
            if (!states.TryGetValue(entity.EntityId, out value) || !value.Entity.Equals(entity)) return false;
        ...
        private byte[] EncodeCanonical()
        {
            using (MemoryStream stream = new MemoryStream())
            {
                ...
                writer.Write(0x32424746u);
                writer.Write((uint)states.Count);
                foreach (BuildingStateV2 state in states.Values)
                {
                    writer.Write(state.Entity.EntityId); writer.Write(state.Entity.Generation);
    ...
    public static class BuildingDomainCodecV2
    {
        public static byte[] EncodeIntent(BuildingIntentV2 value)
        {
            ...
            using (MemoryStream stream = new MemoryStream())
            {
                ...
                {
                    WriteString(writer, value.PrefabKey); writer.Write(value.X); writer.Write(value.Y); writer.Write(value.Z);
                    writer.Write(value.Angle); writer.Write(value.Length); writer.Write(value.ConstructionCost);
                }
        ...
        public static BuildingIntentV2 DecodeIntent(byte[] bytes)
        {
            ...
            using (BinaryReader reader = new BinaryReader(new MemoryStream(bytes, false)))
            {
                ...
                if (kind == BuildingIntentKindV2.Create)
                    result = BuildingIntentV2.Create(ReadString(reader), reader.ReadSingle(), reader.ReadSingle(), reader.ReadSingle(), reader.ReadSingle(), reader.ReadByte(), reader.ReadInt32());
                else if (kind == BuildingIntentKindV2.Delete)
                    result = BuildingIntentV2.Delete(new EntityIdentityV2(reader.ReadUInt64(), reader.ReadUInt32()));
                else throw new InvalidDataException("Unknown building intent kind.");
        ...
        public static byte[] EncodeResult(BuildingResultV2 value)
        {
            ...
            using (MemoryStream stream = new MemoryStream())
            {
                ...
                if (value.Kind == BuildingResultKindV2.Created)
                {
                    ...
                    writer.Write(value.State.Angle); writer.Write(value.State.Length); writer.Write(value.State.BuildIndex); writer.Write(value.State.ConstructionCost);
                }
                writer.Flush(); return stream.ToArray();
            }
        }
        ...
        {
            if (bytes == null || bytes.Length < 13 || bytes.Length > 264) throw new InvalidDataException("Invalid building result length.");
            using (BinaryReader reader = new BinaryReader(new MemoryStream(bytes, false)))
            {
                BuildingResultKindV2 kind = (BuildingResultKindV2)reader.ReadByte();
                EntityIdentityV2 entity = new EntityIdentityV2(reader.ReadUInt64(), reader.ReadUInt32()); BuildingResultV2 result;
                if (kind == BuildingResultKindV2.Created)
                {
                    string prefab = ReadString(reader);
                    result = BuildingResultV2.Created(new BuildingStateV2(entity, prefab, reader.ReadSingle(), reader.ReadSingle(), reader.ReadSingle(),
                        reader.ReadSingle(), reader.ReadByte(), reader.ReadUInt32(), reader.ReadInt32()));
                }
        ...
        private static void WriteString(BinaryWriter writer, string value)
        {
            ...
            if (bytes.Length > 192) throw new InvalidDataException("Building prefab identity is too long.");
            writer.Write((ushort)bytes.Length); writer.Write(bytes);
        }
        private static string ReadString(BinaryReader reader)
        {
            ushort length = reader.ReadUInt16(); if (length == 0 || length > 192) throw new InvalidDataException("Invalid building prefab identity length.");
            byte[] bytes = reader.ReadBytes(length); if (bytes.Length != length) throw new EndOfStreamException(); return Encoding.UTF8.GetString(bytes);
        }
        private static void EnsureEnd(BinaryReader reader)
        {
            if (reader.BaseStream.Position != reader.BaseStream.Length) throw new InvalidDataException("Unexpected trailing building bytes.");
        }
    }
}
