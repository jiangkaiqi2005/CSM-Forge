...
namespace CsmForge.Core
{
    ...
    public sealed class BuildingIntentV2
    {
        ...
        public static BuildingIntentV2 Create(string prefabKey, float x, float y, float z, float angle, byte length)
        {
            ...
            return new BuildingIntentV2 { Kind = BuildingIntentKindV2.Create, PrefabKey = prefabKey,
                X = x, Y = y, Z = z, Angle = angle, Length = length };
        }
    ...
    public static class BuildingDomainCodecV2
    {
        public static byte[] EncodeIntent(BuildingIntentV2 value)
        {
            ...
            using (MemoryStream stream = new MemoryStream())
            {
                ...
                if (value.Kind == BuildingIntentKindV2.Create)
                {
                    ...
                    writer.Write(value.Angle); writer.Write(value.Length);
                }
        ...
        {
            if (bytes == null || bytes.Length == 0 || bytes.Length > 256) throw new InvalidDataException("Invalid building intent length.");
            using (BinaryReader reader = new BinaryReader(new MemoryStream(bytes, false)))
            {
                BuildingIntentKindV2 kind = (BuildingIntentKindV2)reader.ReadByte();
                BuildingIntentV2 result;
                if (kind == BuildingIntentKindV2.Create)
                    result = BuildingIntentV2.Create(ReadString(reader), reader.ReadSingle(), reader.ReadSingle(), reader.ReadSingle(), reader.ReadSingle(), reader.ReadByte());
