...
| TM:PE 11.9.4.1 | 1637663252 | Dedicated synchronized adapter | P6-A `bridge.tmpe-persistent-rules` 与 P6-B/P7 exact-version dynamic barrier/result adapters 已实现：持久规则转 Stable Node/Segment/Lane，Path route 转 Stable Path/Segment，Vehicle/CitizenInstance 转 Forge stable lifecycle/coarse result；原生槽位不进 wire。当前仍是 `blocked-mod`：TM:PE UI 写入口和真实双机行为证据尚未闭合，不能把 CI green 写成 Supported。见 `TMPE-PERSISTENT-RULES-AUDIT-V1.zh-CN.md`、`TMPE-DYNAMIC-AUTHORITY-AUDIT-V1.zh-CN.md` |
